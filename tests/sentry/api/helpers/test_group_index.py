...
class ValidateSearchFilterPermissionsTest(TestCase):
    ...
    def test_negative(self):
        query = '!has:user'
        with self.assertRaisesRegexp(ValidationError, '.*negative search.*'):
            self.run_test(query)

        with self.feature('organizations:advanced_search'):
            self.run_test(query)
    ...
    def test_wildcard(self):
        query = 'abc:hello*'
        with self.assertRaisesRegexp(ValidationError, '.*wildcard search.*'):
            self.run_test(query)
